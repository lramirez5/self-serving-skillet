...
export function ListDrinkRecipesComponent() {
    ...
    async function fetchPosts() {
        const apiData = await API.graphql({ query: categoryByDate, variables: { category: "drinkrecipe" } });
        const postsFromAPI = apiData.data.categoryByDate.items;
        console.log(postsFromAPI[0].image);
